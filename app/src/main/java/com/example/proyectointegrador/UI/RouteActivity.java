...
public class RouteActivity extends AppCompatActivity implements SignalRService.SignalRListener {
        ...
        new Thread(() -> {
            try {
                ...
                if (routeResponseCode == 200) {
                    ...
                    String routeLine;
                    while ((routeLine = routeReader.readLine()) != null) routeResponse.append(routeLine);
                    routeReader.close();
    ...
    private void cargarYMostrarRuta() {
        new Thread(() -> {
            try {
                ...
                android.util.Log.d("RouteActivity", "📍 Cargando detalles del trip: " + tripDetailUrl);
                ...
                android.util.Log.d("RouteActivity", "📍 Response Code: " + responseCode);
                ...
                if (responseCode == 200) {
                    ...
                    android.util.Log.d("RouteActivity", "📍 Response: " + tripResponse);
                    ...
                    if (routeGeometry != null && !routeGeometry.isEmpty() && !routeGeometry.equals("null")) {
                        android.util.Log.d("RouteActivity", "✅ RouteGeometry encontrada, dibujando en mapa");
                        runOnUiThread(() -> dibujarRutaEnMapa(routeGeometry));
                    } else {
                        android.util.Log.w("RouteActivity", "⚠️ No hay geometría de ruta disponible");
                        runOnUiThread(() -> {
                            Toast.makeText(this, "No hay geometría de ruta disponible", Toast.LENGTH_SHORT).show();
                            mostrarMapaDefault();
                        });
                    ...
                    android.util.Log.e("RouteActivity", "❌ Error " + responseCode + ": " + errorMessage);
                    
                    runOnUiThread(() -> {
                        Toast.makeText(this, "Error al cargar el viaje: código " + responseCode, Toast.LENGTH_SHORT).show();
                        mostrarMapaDefault();
                    });
                }
            } catch (Exception e) {
                e.printStackTrace();
                android.util.Log.e("RouteActivity", "❌ Exception al cargar ruta: " + e.getMessage(), e);
                runOnUiThread(() -> {
    ...
    private List<Point> decodePolyline(String encoded) {
        ...
        while (index < len) {
            ...
            // Polyline con precisión de 6 decimales (1E6) - nuestro polyline usa esta precisión
            Point p = Point.fromLngLat((double) lng / 1E6, (double) lat / 1E6);
            poly.add(p);
        }
    ...
    private void dibujarRutaEnMapa(String polylineString) {
        try {
            ...
            // Mostrar los primeros 3 puntos para verificar
            ...
            mapboxMap.loadStyleUri("mapbox://styles/mapbox/streets-v12", style -> {
                ...
                try {
                    ...
                    com.mapbox.maps.extension.style.layers.generated.SymbolLayer startLayer = 
                        new com.mapbox.maps.extension.style.layers.generated.SymbolLayer("start-layer", "start-source");
                    startLayer.iconImage("start-marker-icon");
                    startLayer.iconSize(1.5);
                    startLayer.iconAnchor(IconAnchor.BOTTOM);
                    LayerUtils.addLayer(style, startLayer);
                    ...
                    com.mapbox.maps.extension.style.layers.generated.SymbolLayer endLayer = 
                        new com.mapbox.maps.extension.style.layers.generated.SymbolLayer("end-layer", "end-source");
                    endLayer.iconImage("end-marker-icon");
                    endLayer.iconSize(1.5);
                    endLayer.iconAnchor(IconAnchor.BOTTOM);
                    LayerUtils.addLayer(style, endLayer);
                    
                    // Crear el marcador del bus (con diseño mejorado)
                    ...
                    busShadowLayer.circleColor(Color.parseColor("#40000000")); // Sombra
                    busShadowLayer.circleOpacity(0.3f);
                    LayerUtils.addLayer(style, busShadowLayer);
                    ...
                    busOuterLayer.circleColor(Color.parseColor("#8D0743")); // Color principal
                    LayerUtils.addLayer(style, busOuterLayer);
                    ...
                    LayerUtils.addLayer(style, busRingLayer);
                    
                    // Círculo interior del bus
                    CircleLayer busLayer = new CircleLayer("bus-layer", "bus-source");
                    busLayer.circleRadius(10.0);
                    busLayer.circleColor(Color.parseColor("#B31D5A")); // Color vibrante
                    LayerUtils.addLayer(style, busLayer);
    ...
    public void onBusLocationReceived(String busId, double latitude, double longitude, String status) {
        // Actualizar posición del bus en tiempo real
        android.util.Log.d("RouteActivity", String.format("📍 onBusLocationReceived: busId=%s, lat=%f, lng=%f", busId, latitude, longitude));
        ...
        runOnUiThread(() -> {
            ...
            mapView.getMapboxMap().getStyle(style -> {
                try {
                    ...
                    // Mover cámara suavemente al bus
    ...
    public void onTicketScanned(String scannedTicketId, String scannedTripId, String studentId) {
        // Verificar si es nuestro ticket
        ...
        new Thread(() -> {
            try {
                ...
                if (conn.getResponseCode() == 200) {
                    ...
                    runOnUiThread(() -> {
                        if (wasScanned) {
                            // Ticket ya fue escaneado, mostrar ruta
                            ticketWasScanned = true;
                            waitingOverlay.setVisibility(android.view.View.GONE);
                            cargarYMostrarRuta();
                        } else {
                            // Ticket no escaneado, mostrar overlay de espera
